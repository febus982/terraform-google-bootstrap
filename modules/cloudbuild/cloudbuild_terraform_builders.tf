...
variable "builders_tag_map" {
  type    = map(object({
      ...
      terraform_validator_release = string,
  }))
  default = {
    "0.12" = {
...
resource "null_resource" "custom_cloudbuild_terraform_builder" {
  ...
  provisioner "local-exec" {
    command = <<EOT
      ...
      --substitutions=_IMAGE_TAG=${each.key},_TERRAFORM_VERSION=${each.value.terraform_version},_TERRAFORM_VERSION_SHA256SUM=${each.value.terraform_version_sha256sum},_TERRAFORM_VALIDATOR_RELEASE=${each.value.terraform_validator_release}
  EOT
  }
  depends_on = [
